...
def add_points(frame_1, frame_2, corner_storage, view_mats, point_cloud_builder, intrinsic_mat):
    ...
    cors = build_correspondences(
        corner_storage[frame_1], corner_storage[frame_2], point_cloud_builder.ids)
    if (len(cors) == 0):
        return
...
def track_camera(corner_storage, intrinsic_mat, known_view_1, known_view_2):
    ...
    start_frame_1 = min(known_view_1[0], known_view_2[0])
    start_frame_2 = max(known_view_1[0], known_view_2[0])
    processing_frame = start_frame_1 + 1
    outliers = np.zeros(
    ...
    add_points(last_frame, known_view_2[0], corner_storage,
               view_mats, point_cloud_builder, intrinsic_mat)

    for _ in range(2, total_frames):
        if processing_frame == start_frame_2 and processing_frame < total_frames - 1:
            processing_frame += 1
        ...
        last_frame_dist = 5
        while counter < 5:
            if processing_frame > start_frame_1:
