...
class CornerTracker:
    ...
    def __init__(self, frame):
        ...
        self.quality = 0.008
        self.min_dist = 4
        self.corners, self.radiuses = self.find_corners(
            frame, max_corners=self.max_corners, quality=0.002)
    ...
    def process_frame(self, new_frame):
        ...
        if self.corners.shape[0] < self.max_corners:
            new_corners, new_radiuses = self.find_corners(new_frame, filter_,
                                                          ...
                                                          self.corners.shape[0],
                                                          quality=self.quality)
            new_ids = np.arange(
                self.last_id, self.last_id + new_corners.shape[0])
    ...
    def find_corners(self, frame, filter_=None, pyr_level=3, point_size=4, max_corners=10000, quality=0.008,
        ...
        for i in range(pyr_level):
            ...
            new_filter = self.create_filter(
                frame_, new_corners / coef, area_size=min_dist)
    ...
    def apply_opt_flow(self, new_frame, corners, max_level=2):
        ...
        dists = abs(corners - pb).squeeze().max(axis=1) < 0.4
...
def _build_impl(frame_sequence: pims.FramesSequence,
    ...
    builder.set_corners_at_frame(
        0, FrameCorners(corners_ids, corners, radiuses))
